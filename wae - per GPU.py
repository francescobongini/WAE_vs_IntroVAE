...
class WAE(object):
    ...
    def crea_pics(self,sample_test,recon_test):
        img=[]
        sample_test = sample_test / 2. + 0.5
        recon_test = recon_test / 2. + 0.5
        for pair in [(sample_test, recon_test)]:
            ...
            r_ptr = 0
            w_ptr = 0
            for _ in range(len(sample_test)//2):
    ...
    def salva_pics(self,i,pics,j):
        ...
        #img9=data.test_data[0][0] #sicuramente questo doppio 0 non va bene
        img9= pics[i]
        
        
        for img, (gi, gj, title) in zip([img9],
                                 [(0, 0, 'a')]):
        ...
        b='./risultati'
        utils.create_dir(b)
        fig.savefig(utils.o_gfile((b, filename), 'wb'),
                    dpi=dpi, format='png')


    def salva_pics2(self,i,pics,j):
        dpi = 100
        #height_pic = img1.shape[0]
        #width_pic = img1.shape[1]
        #fig_height = 4 * height_pic / float(dpi)
    ...
    def salva_test(self,j):
        for i in range(0,100,2):
            self.salva_pics(i,self.pics,j)

    def salva_gen_test(self,j):
        for i in range(1,100,2):
            self.salva_pics2(i,self.pics,j)
    ...
    def train(self, data):
        ...
        for epoch in xrange(opts["epoch_num"]):
            ...
            for it in xrange(batches_num):
                ...
                if counter % opts['print_every'] == 0:
                    ...
                    # Making plots
                    #self.num_pics=int(self.num_pics)
        ...
        for i in range(0,20000,100):
            train_set=data.data[:self.num_pics]
            [loss_rec_train, enc_train, rec_train] = self.sess.run(
                    [self.loss_reconstruct, self.encoded, self.reconstructed],
                    feed_dict={self.sample_points: data.data[:self.num_pics],
                               self.is_training: False})                        
            self.pics=self.crea_pics(train_set, rec_train)
            self.salva_test(i)
            self.salva_gen_test(i)
